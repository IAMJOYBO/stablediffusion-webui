...
RUN echo "sudoaccount ALL= (ALL:ALL) NOPASSWD: ALL" >> /etc/sudoers
WORKDIR /app
RUN groupadd -r sd-webui && useradd -r -g sd-webui sd-webui
USER sd-webui

RUN sudo apt update && sudo apt upgrade -y && sudo apt install -y wget curl net-tools tree git git-lfs iputils-ping
